...
## PG-5設計方針（2026-07-20確定・実装は本フェーズ着手時）

独自ドメインの持ち主はPageサイトではなく **mikkeID** とする。ドメイン→mikkeIDの
紐づけを共通テーブル（例: custom_domains）で持ち、mikkeIDの持ち主が「そのドメインで
何を表示するか」を選べるようにする。選べる対象はPageサイトに限定せず、Academyの公開LP
（`/academy/c/[id]`）やFundの公開プロジェクトページなど、各アプリの既存公開ルートも
候補にする。

理由: Academy・Fundなど「そのアプリ単体が本体」の業者（例: 認定講座構築）にとって、
独自ドメインが常にPage経由になる設計だと、Pageサイトを作ってその中にCMSブロックで
埋め込むという遠回りが必須になってしまう。ドメインの持ち主をmikkeID単位にしておけば、
「独自ドメイン→Academyの公開LPそのもの」も「独自ドメイン→Pageサイト（中でAcademyや
Fundを紹介）」も同じ仕組みで選べる。Pageだけを特別扱いしない。

実装への影響: PG-5着手時、ドメイン所有テーブルをPage専用スキーマではなく共通スキーマ
として設計する（Fund F4のidentity/consent基盤と同じ「Page専用に閉じない」考え方）。
今回は方針の記録のみ・実装はしない。
```
...
現時点のCMS参照元は Story / Item Studio / Event / Academy / Session / Order / FUND / Community とし、Connect / Partners はこれらを組み合わせて作れるかを検証する構築対象として扱う。

そのため、Page側に Connect / Partners 専用の保存型・追加フォーム・管理画面・独立アプリ導線を作らない。

## 2026-07-19 Page Builder 拡張方針

Pageは、CMS連携だけでなく、日常的な会社・店舗・活動サイトを感覚的に作れるホームページ作成アプリとして育てる。
STUDIOより操作を絞りつつ、テンプレートとブロックを組み替える自由度を持たせる。

今回の編集基盤では、会社概要、画像と文章、2・3カラム、画像グリッド、スライドショー、CTA、フォーム表示、CMS、外部埋め込みHTML、自由HTMLを扱う。
サイト全体では、配色、見出し・本文フォント、横幅、ボタン形状を選択でき、各ブロックには余白、角丸、文字揃え、簡単な表示アニメーションを設定できる。

AIが作ったHTMLを一式表示するページモードも用意する。ただし、入力HTMLはPage編集画面のDOMへ直接挿入せず、CSP付きのsandbox iframe内に隔離する。スクリプト実行はページ単位で利用者が明示的に許可した場合だけ有効にする。

画像本体はブラウザ保存やPageデータへ埋め込まず、Supabase Storageの `page-assets` バケットへ保存する。アップロード前にWebPへ圧縮・長辺縮小し、Pageデータには公開URLと表示用メタデータだけを保持する。
公開URLを使うためPageデータは軽量に保てるが、閲覧時には画像通信が発生するため、遅延読み込みと適切な画像サイズを継続して使う。

公開ルート、他者掲載依頼、Manager受信箱接続、決済、独自ドメインは、この拡張には含めない。

...
```text
各Mikkeアプリ = 入口・作業場所
Manager       = 各アプリの予定・進行・履歴を横断で参照し、次に使えそうなアプリを提案する場所（Mikke IDに付属）
Activity Log  = 裏側の記録エンジン（表に出さない）
Story         = 公開する実績
```

### 1.1 体験方針（2026-07-18 ユーザー確認で確定）

Managerは、ユーザーが最初に作業を始める「巨大なOSホーム」ではない。

```text
確定:
- 入口はあくまで個々のアプリ。
  例: 出店ならMarketNote、依頼受付ならOrder、相談予約ならSession、
      支援募集ならFund、チーム運営ならTeam Works。
- Managerは、各アプリで進んでいる予定・タスク・進行を横断で参照する場所。
- Managerは「次にやること」を見せる。
- Managerは「他のアプリで進んでいること」を見せる。
- Managerは状況に応じて「こんなアプリもあるよ」と提案する。
- Managerをログイン直後の唯一の玄関にしない。
```

このため、2.1章の「M1完了後に `/` とlogin後を `/manager` へ変更する」は撤回する。
`/os` は引き続き凍結対象だが、入口の置き換え先をManagerと決め打ちしない。
玄関設計は「アプリファースト」に揃える。
...
- Manager M1完了後、loginリダイレクトを /os → /manager へ変更する案は撤回。
  Managerはログイン直後の巨大な玄関ではなく、各アプリから参照する横断面とする。
- （2026-07-15追記・動線監査）リダイレクトは2箇所ある。login（app/login/page.tsx）
  に加えて、ルート / も app/page.tsx で /os へ固定リダイレクトしている。
  M1完了時に両方を /manager へ変更する案は撤回。
  入口は個々のアプリに置き、Managerは参照・提案の場所にする。
...
- M1ではナビを変更しない。Manager入口はMikkeOwnerMenuの先頭項目
  ...
  ただし意味は「作業開始の玄関」ではなく「各アプリの動きを横断で見る場所」。
- ボトムナビ「OS」枠を単純に「Manager」へ置き換える案は保留。
  Managerを全体玄関に見せすぎるため、アプリファーストの動線設計と
  矛盾しない形をM2で再設計する。
```
...
```text
/manager            ホーム（今日の予定・今週の締切・対応が必要・進行中・最近の活動・次に使えそうなアプリ）
...
Managerのホームは、アプリ一覧を中心に置かない。あくまで各アプリで発生した
予定・タスク・進行を見たうえで、文脈に合うアプリだけを控えめに提案する。
...
- ボトムナビOS枠の再設計。
  単純なManager置き換えではなく、入口は個々のアプリ、Managerは横断参照という
  体験方針を崩さないナビにする。

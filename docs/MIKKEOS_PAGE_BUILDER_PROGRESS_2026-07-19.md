...
- ダッシュボードをテンプレート起点に再構成し、用途ごとの見た目をカードで比較できるようにした
- 新規作成画面でもテンプレートの縮小見本を表示し、ダッシュボードで選んだテンプレートを引き継ぐ
- PC編集は「追加・デザイン / 大きなキャンバス / 選択箇所の編集」の3領域とし、キャンバスを中心にした
- プレビュー内のブロックを直接選択し、選んだ部分だけを右側で編集する
- PC・タブレット・スマホを実寸比率で切り替え、50 / 60 / 75 / 100%の倍率で確認できる
- タブレット編集はプレビューと編集を並べ、追加時だけ追加パネルへ切り替える
- スマホ編集は「見る / 編集 / 追加・デザイン」を1画面ずつ切り替え、スマホだけでも全操作を完結できる
- AI HTMLモードは従来どおり独立したコード編集と安全プレビューを維持する

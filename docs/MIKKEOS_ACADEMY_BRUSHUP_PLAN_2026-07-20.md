...
## 16. Wave G（2026-07-20・第3弾FB＝Academyの中核再設計）

あゆみのスクショ9枚＋要望を受けた設計。**これはUI微修正ではなく、Academyの
講座ページ／フロントページを「1つの統合ブロックビルダー」へ作り替える中核変更**。
あゆみが何度も言っていた本質を、今回ようやく設計に落とす。

### G0. 何度も言われていた本質（Fableの理解）

```text
これまで: 構造化フォーム（受講料・キット内容・カリキュラム・FAQ…固定順で描画）
          ＋ 別画面のLPビルダー（末尾に追加されるだけ）＝2画面・順番固定。
問題点（あゆみの言葉）:
  - 講座情報の欄とビルダーの欄を「入れ替えたい」「表示を自由にしたい」
  - キット内容が膨大だと、延々スクロールしてからPR(ビルダー)に入る
  - 「講座をしっかり見せて、最後にキット内容やカリキュラム」の自由が欲しい
  - フロントも固定エリアが世界観を消す→ブロックを移動したい
  - 目標は9枚目（本番CACM/MUSUBIサイト）レベルの見せ方（何度も言っている）
実コード確認済み: c/[id]は
  受講後にできること→認定条件→キット内容→教材内容→lp_blocks→FAQ の完全固定順。
```

### G1. 統合ビルダー（講座ページの中核再設計）— 最重要

```text
方針: 構造化データは業務ロジックのため残す（price=売上, kit_price=発注,
      formats=フロー分岐に使う）。だが「表示」は1本のブロック列にする。
      → Pageの「CMSブロックがアプリのデータを参照する」パターンをAcademy内に適用。

セクションブロック（構造化データを参照して描画・順序自由）:
  course-summary  受講料/受講時間/受講形式＋申込ボタン
  can-do-after    受講後にできること
  certification   認定条件
  kit-contents    キット内容
  curriculum      教材内容/カリキュラム
  faq             よくあるご質問
自由ブロック（Wave C資産＋miraco参照で追加）:
  heading / text / image / image-text(画像+文章) / gallery(画像グリッド) / cta
  ＋ checklist（チェックリスト）/ steps（制作の流れ・番号付き）/ columns（カラム）
  （miracoのブロックパレットに合わせる。PC対応はカラム＋画像+文章で網羅）

画面統合: /academy/courses/[id]（講座情報編集）と /academy/courses/[id]/lp
  （LPビルダー）を1画面に統合する。上部＝業務データのフォーム（コード/名称/
  受講料/キット代金/formats/申込項目など、順序に関係ない設定）、下部＝表示ブロックの
  並べ替えエリア（セクションブロック＋自由ブロックを1つのリストで上下移動）。
  タブ切替でも良いが「同じ画面で完結」を最優先（あゆみ明言）。

データ: course.lp_blocks を拡張し、セクションブロックも格納できる型にする。
  新規講座作成時に「妥当な初期並び」をseed（今の固定順を初期値にすれば、
  何もしなくても従来通り→そこから自由に並べ替え）。既存講座は
  lp_blocksにセクションブロックが無いので、描画時に「構造化フィールドを
  従来順で自動補完」するフォールバックを入れる（旧データ互換）。
```

### G2. フロントも同じビルダー＋世界観（テーマ）

```text
- フロントのヒーロー（アカデミー名/キャッチ/紹介文/画像）も固定エリアをやめ、
  ブロック化して移動可能に（Wave Fでfront_blocksは追加済み。ヒーローもblock化）。
- 「素敵にならない」問題の本質: プレーンに積むだけでは9枚目にならない。
  必要なのは (a)リッチなブロック (b)ブランドテーマ（色・余白・セクション背景）
  (c)良い初期テンプレート。G1のブロック＋G3のブランドカラー＋seedテンプレで
  「箱から出してそれなりに整う」状態を作る。
  ※正直な線引き: 9枚目は専用Webツール(STUDIO等)の手作りサイト。1:1完全再現は
  目標にしない。目標は「団体の世界観で・整っていて・魅力的」。ここは期待値を
  すり合わせた上で、到達可能な最大限を目指す。
- ブロックに簡易スタイル（背景色/余白/中央寄せ）を持たせる（PageのPageBlockStyle
  相当。テーマ色を各ブロックが継承）。
```

### G3. 団体ブランディング設定（新規・設定画面）

```text
- 新規「設定」画面（本部）: アカデミー名変更・団体ロゴ画像・ブランドカラー。
  academy_headquarters に logo_url text / brand_color text を追加（要SQL）。
- ブランドカラー/ロゴが フロント・講座ページ・講師ポータルのヘッダーに反映
  （世界観の統一＝G2の「素敵にならない」への効き目が大きい）。
```

### G4. 教えっぱなし（キット無し）団体モード

```text
Wave FでrequiresKit=falseはキットUI/ステータスを隠すところまで実装済み。今回追加:
- キット無し運用では講師ポータルの「営業ページ（営業用URL）」を出さない切替。
  見せるのは「講師ページ（復習）」のみ。
- 「講師ページ」という名称を団体ごとに変えられるようにする（団体により呼び方が
  違う）。academy_headquarters に instructor_page_label text（既定「講師ページ」）。
  ※要確認: 「営業ページ無し」は講座単位(requires_kit)に連動させるか、団体単位の
  独立設定にするか。→ 提案: 団体単位の「講師が営業して集客するモデルか」トグルを
  設定画面に置く（キット有無とは別軸。キット売るが営業はしない団体もありうるため）。
```

### G5. 申込フォームビルダー強化（miracoパリティ）

```text
miracoの申込フォーム（スクショ2）は選択肢ごとに +¥（価格加算）＋画像を持てる。
Academyの現状AcademyFormFieldは text/textarea/email/tel/select/checkbox のみ。
- 選択肢型（radio/select/checkbox）のoption に price_add（加算額）と image_url を
  追加できるようにする（例: デュプロマ追加オプション+¥など）。
- ラジオボタン型を追加（現状select/checkboxのみ）。
※これは大きめなので、G1〜G4の後の別スライスにしてよい。優先度は中。
```

### G6. 小さな修正群

```text
G6-a 講師写真が表示されない（バグ）: 保存はphoto_urlに入るが、講師カード
     （一覧app/academy/instructors/page.tsx・編集画面ヘッダーカード・公開
     app/academy/i/[id]）がphoto_urlを描画していない。表示側を追加。
G6-b ダッシュボードのパネル群をリンク化（講座数→講座管理, 認定講師→講師管理,
     今月の申込→申込管理 等）。
G6-c 料金に税・送料の記載: course に price_tax_note / kit_shipping_note 等の
     補足テキスト欄を追加（税込/税別・送料の扱いを明記できる）。要SQL。
G6-d 更新期限まわり（あゆみの質問への回答＋改善）: 現状、更新しない講師は
     「活動ステータス=休止」に変え、講師一覧掲載/受付有効のチェックを外す運用。
     改善案: 更新期限を過ぎた講師に「更新期限切れ」バッジを自動表示、任意で
     期限超過を一覧でフィルタ。自動休止はしない（本部の判断を残す）。
```

### 要SQL（G3/G4/G6c分・実装時に§9へ追記）

```text
academy_headquarters: logo_url text / brand_color text /
                      instructor_page_label text default '講師ページ' /
                      instructor_sales_enabled boolean default true
academy_courses:      price_tax_note text / kit_shipping_note text
```
（G1の統合ビルダーはlp_blocksの型拡張のみでスキーマ変更不要の見込み）

### 実装順・スライス（大きいので分割）

```text
G1（統合ビルダー・セクションブロック・画面統合）＝最重要・最初
G3（ブランディング設定）＋G2（フロントのブロック化・テーマ反映）
G6（小修正群・写真バグは即効性ありG1と並行可）
G4（教えっぱなしモード）
G5（フォームビルダー強化）＝最後
```
...
```text
1. G1の方向性: 「講座情報も含めて全部ブロック化し、1画面で自由に並べ替え」で
   合っているか（構造化データ自体は業務のため裏で保持しつつ、表示だけ自由に）。
2. G4: 「営業ページ無し」は 団体単位の独立トグル で良いか
   （キット有無とは別軸にする提案）。
3. 期待値: 9枚目レベルは専用Webツールの手作り。1:1ではなく「団体の世界観で
   整った魅力的なページ」を到達目標とする線引きで進めてよいか。
```

## 17. Wave G 確定事項（2026-07-20・あゆみ確認済み・実装へ）

3点とも確認済み。以下を反映して実装へ進む。

```text
G1（統合ビルダー）: 「合っています」で確定。
G4（教えっぱなしモード）: 団体単位の独立トグルで確定（instructor_sales_enabled）。
期待値: 「1:1ではなく世界観で整った魅力的なページ」で確定。
```

### G3追記: フォント選択

日本語＋英語（欧文）の2軸、Page（`lib/page/fonts.ts`）と同じカタログ・同じ仕組みを
そのまま再利用する（見出し/本文×JP/Latinの4選択、Google Fonts css2動的読み込み）。
Academy専用に作り直さない。academy_headquartersに
`heading_jp_font_id/heading_latin_font_id/body_jp_font_id/body_latin_font_id`
（全てoptional・省略時デフォルト）を追加（要SQL）。フロント・講座公開LP・
講師公開ページに適用。

### G1/G2追記: 画像+文章（image-text）ブロックのレイアウト改善

現状の問題（あゆみ指摘）: 画像+文章ブロックでタイトル・文章が縦方向で
中央寄りに詰まって見える、左右の入れ替えができない。

```text
- image-text ブロックに imagePosition: "left" | "right"（既定"left"）を追加。
  ボタンで左右反転できるようにする。
- テキスト側のレイアウトを「フォーマット化」する:
  見出し（大/中/小のいずれか・上寄せ）→ 少し余白 → 本文（上寄せ、行間ゆったり）。
  縦方向で中央寄せにしない（現状のitems-centerのような詰まった見え方をやめる）。
  この配置ルールは新規に作る全ブロック（image-text/columns等）で共通の
  「Academyブロック標準フォーマット」として扱い、以後増えるブロックにも適用する。
```

### 実装範囲（今回のSonnet依頼）

G1 + G2 + G3（フォント含む）+ G4 + G6 を実装する。**G5（申込フォームの選択肢
価格加算・画像・ラジオボタン追加）は今回スコープ外**（設計はdocsに残したまま、
次回の別スライスとする。理由: 今回のG1だけで規模が大きく、利用枠の都合上
分割する）。

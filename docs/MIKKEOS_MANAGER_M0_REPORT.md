...
ただし、Managerは「新しいOSホーム」でも「ログイン直後の巨大な玄関」でもない。入口はあくまで個々のアプリで、Managerは各アプリの予定・タスク・進行を読み取り変換して束ね、次に見ることや使えそうなアプリを提案する個人用の横断面として扱う。既存の `/os` と `/log` は削除せず、M1中は拡張せずに残置する。
...
- `/os` と `/log` の最終処遇
- ルート `/` と login後遷移先を単純に `/manager` 化する案
- BottomNav全体の置き換え
- Google Calendar接続
- Inbox実装
...
| `/` | `app/page.tsx` で `/os` へ遷移 | M1では変更しない。Managerへの単純置換はしない |
| login後 | `app/login/page.tsx` で未指定時 `/os` へ遷移 | M1では変更しない。Managerへの単純置換はしない |
| `MikkeOwnerMenu` | 表示設定 / Log / Story / DESK / Apps を表示 | M1でManager入口を先頭に追加する候補 |
| `BottomNav` / `MikkeAppShell` | `/os` と `/log` を含む | M1ではナビ構成を変更しない |

このため、M1の導線追加はまずOwnerMenuだけに限定するのが安全。M2では、入口を個々のアプリに置いたまま、Managerを参照・提案の場所として自然に呼び出せる動線を再設計する。
...
- Managerが状況に応じて、次に使えそうなアプリを控えめに提案する

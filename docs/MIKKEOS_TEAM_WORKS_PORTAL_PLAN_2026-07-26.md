...
#### 運用開始日の担当表示・希望シフト・モバイル調整（2026-07-28 codex）

- パートナーポータルの予定取得を、ログイン中の本人に紐づく`partner_member_id`へ明示的に限定。管理者とパートナーを兼ねるテストアカウントでも、他人の担当予定を混在させない。
- パートナーポータルに「シフト提出」を追加。翌月を初期表示し、前月25日を提出期限として案内する。希望日をカレンダーで選択し、希望日数・本部へのメモを提出／更新できる。運用開始直後のため、期限後も提出自体は可能とする。
- 本部ホームの月間カレンダーに希望日を黄色で表示し、同画面の希望シフト一覧で候補日・希望日数・メモを確認できる。本部の「確認済み」は提出内容の受領確認であり、既存スケジュールへの担当確定は自動で行わない。
- スタンバイ通知後は、独立レッスン画面のZoom情報・操作ボタン・Zoom設定を自動で畳む。「操作を表示」で再度開けるため、スマホでは進捗マニュアルの表示領域を広く確保する。
- Team Worksから開く共通メニューには、未稼働のDESK／Apps／「アプリをつなげる」を渡さない。クライアントポータルとパートナーポータル等、稼働中のTeam Works導線だけを表示する。
- migration `20260727225109_team_works_partner_shift_availability.sql`を追加。本人のみ提出、同一組織の本部のみ閲覧・確認できるRLSを設定し、`authenticated`のテーブル権限も`SELECT / INSERT / UPDATE`だけに限定。開発Supabaseへ適用し、本人・他人・本部の権限試験をロールバック付きで確認した。

#### 全ポータル共通の日本休日・カレンダー導線調整（2026-07-28 codex）

- 日本の土曜・日曜・国民の祝日を共通判定する`lib/japanese-calendar.ts`を追加。振替休日・国民の休日・春分／秋分の日を含め、2020・2021年の祝日移動にも対応する。
- 本部ホーム、プロジェクト詳細、クライアント／パートナー共通カレンダー、希望シフト、旧スクールカレンダーで土日祝をオレンジ表示する。祝日は名称も表示し、DBに登録した個別休講とは区別する。
- パートナー上部の「シフト提出」タブを撤去し、総合ホーム内の強調カード「希望シフトを提出」から開く導線へ変更。担当予定が0件でも総合ホームと提出カードを表示する。
- 本部ホームは、現在月に今後の予定がない場合、直近予定がある月を初回だけ自動表示する。クライアント／パートナー共通カレンダーも同じ初期月選択とする。利用者が月移動した後は自動で戻さない。
- 日付クリックの予定詳細は、スマホでは下部シート、PCでは最大幅896pxの中央モーダルへ切り替えた。複数予定はPCで2列表示し、最大高さを超える場合はモーダル内部をスクロールする。

#### 実運用開始日の保存・休校・月移動修正（2026-07-28 codex）

- パートナーレッスン記録から出欠入力を撤去。既存DB列は互換性のため保持し、評価3項目と引継ぎ記録だけで保存できるようにした。
- 別PCの保存失敗をSupabaseログで確認。原因は担当外のログイン利用者が他人のレッスンを表示できていたことによる `You are not assigned to this session`。RPC権限は緩めず、パートナー予定をログイン本人の `partner_member_id` に限定する既存修正で、担当外レッスンを保存画面まで出さない。
- プロジェクト概要カレンダーの前月・次月ボタンを年月の左右へ移動。月変更直後に選択日の月へ戻していた副作用を削除し、次月へ実際に移動できることをブラウザで確認した。クライアント／パートナー共通カレンダーも同じ年月＋左右矢印配置へ統一。
- 土日祝を全ポータルで「休校」扱いに統一。既存レッスンは本部・パートナー・クライアントの予定／注意対象から除外し、今後の定期生成でも作成しない。手動の新規登録・日付移動も土日祝を拒否する。
- 検証: Supabaseの関数権限（authenticatedのみ実行可）とエラーログ確認、`npm.cmd run lint`、`npm.cmd run build`（99 routes）成功。localhostで本部カレンダー、プロジェクト次月移動、山の日の休校表示・予定追加非表示、パートナーポータル本人担当0件、ブラウザエラー0件を確認。

  ...
  for (const [uid, group] of grouped) {
    ...
    if (masters.length === 0) {
      for (const event of exceptionByKey.values()) {
        ...
        if (item && item.dateKey >= from && item.dateKey <= to) {
    ...
    for (const master of masters) {
      if (master.status === "cancelled") {
        if (master.rrule || master.rdates.length) {
          warnings.add("取消された繰り返し予定は安全に特定できないため除外しました。");
          skippedCount += 1;
          continue;
        }
        const cancelled = eventToItem(uid, master, calendar, warnings);
        if (cancelled && cancelled.dateKey >= from && cancelled.dateKey <= to && takeOccurrence(limitState, warnings)) {
      ...
      for (const baseItem of expanded) {
        ...
        if (exception.status === "cancelled") {
          items.push({ ...baseItem, title: exception.title || baseItem.title, status: "cancelled" });
          continue;
        }

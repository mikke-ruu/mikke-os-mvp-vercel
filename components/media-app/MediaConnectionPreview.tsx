...
export function MediaConnectionPreview() {
  ...
  return <div className="mx-auto max-w-4xl">
    ...
    <section className="py-7"><h2 className="font-bold">取り下げたときの動きも確認できます</h2><p className="mt-2 text-sm leading-7 text-[var(--mikke-muted)]">非公開にすると記事欄から消えます。再公開しても自動では戻りません。もう一度記事を選ぶと表示されます。</p><button type="button" className={`${button} mt-4`} onClick={()=>{if(published)setPublished(false);else {setCycle(value=>value+1);setPublished(true);}}}>{published?"見本記事を非公開にする":"見本記事を再公開する"}</button></section>
  </div>;
}

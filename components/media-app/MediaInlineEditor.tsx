...
import type {MediaBlock,MediaBlockType} from "@/lib/media-app/types";
import {createMediaBlock} from "@/lib/media-app/store";
import {MikkeInsertMenu as MediaInsertMenu} from "@/components/mikkeos/content/MikkeInsertMenu";

export function MediaInlineEditor({blocks,onChange,renderBlock,onSaveSnippet,allowedTypes}:{allowedTypes?:MediaBlockType[];blocks:MediaBlock[];onChange:(blocks:MediaBlock[])=>void;renderBlock:(block:MediaBlock,onChange:(next:MediaBlock)=>void,onSplit:(start:number,end:number)=>void)=>ReactNode;onSaveSnippet?:(block:MediaBlock)=>void}) {
  ...
  function splitParagraph(id:string,start:number,end:number) {const next=createMediaBlock("paragraph");focusBlock.current=next.id;onChangeItems(items=>{const index=items.findIndex(item=>item.id===id);if(index<0)return items;const current=items[index];next.text=(current.text??"").slice(end);return [...items.slice(0,index),{...current,text:(current.text??"").slice(0,start)},next,...items.slice(index+1)];});}
 ...
 return (<section aria-label="記事本文">{blocks.map((block, index) => <Fragment key={block.id}><MediaInsertMenu allowedTypes={allowedTypes??["paragraph","heading","image","quote","list","divider","link"]} position={index} onInsert={(type,level)=>insertBlock(index,type as MediaBlockType,level)}/><article id={`media-block-${block.id}`} className="group relative rounded-xl border border-transparent px-2 py-1 focus-within:border-[var(--mikke-line)] sm:px-4">
        ...
        </article></Fragment>)}<MediaInsertMenu allowedTypes={allowedTypes??["paragraph","heading","image","quote","list","divider","link"]} position={blocks.length} onInsert={(type,level)=>insertBlock(blocks.length,type as MediaBlockType,level)}/></section>);
}

...
export function MediaAppShell({ children }: { children: React.ReactNode }) {
  ...
  return <MikkeAppShell appName="Media" title="Media" theme="blue" navItems={navItems.map((item) => ({ ...item, href: href(item.href) }))} bottomNavItems={navItems.map((item) => ({ label: item.label, href: href(item.href), icon: item.icon, primary: item.href.endsWith("/write") }))} primaryActionTone="orange" showSharedUtilities={!reviewing} showBottomNavLabels><div className={styles.content}>{!reviewing?<details className="mb-3 border-b border-[var(--mikke-line)] pb-2 text-xs"><summary className="cursor-pointer">ログイン中：{profile.display_name || profile.handle}</summary><p className="mt-2 break-all">mikkeID：{profile.handle || "未設定"}</p>{user.email?<p className="mt-1 break-all">{user.email}</p>:null}</details>:null}{reviewing ? <p className="mb-5 border-b border-[var(--mikke-line)] pb-3 text-xs leading-6 text-[var(--mikke-muted)]">デザイン確認用・このブラウザ内だけに保存します。外部には公開されません。</p> : null}{children}</div></MikkeAppShell>;
}

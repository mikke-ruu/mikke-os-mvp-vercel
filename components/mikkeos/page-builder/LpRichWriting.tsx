...
import { Type, Bold, Strikethrough, Link as LinkIcon, AlignLeft, Settings2 } from "lucide-react";
import styles from "./rich-writing.module.css";
...
export function LpRichWriting({block,onChange,onSplit,onUrlPaste,compact=false,onDesign}:{block:MediaBlock;onChange:(value:MediaBlock)=>void;onSplit?:(start:number,end:number)=>void;onUrlPaste?:(url:string)=>void;compact?:boolean;onDesign?:()=>void}) {
  const [menu,setMenu]=useState<"type"|"align"|null>(null);
  ...
  return <div onFocus={()=>setActive(true)} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget))setActive(false);}}>
    {active?<div role="toolbar" aria-label="文章の装飾" className={compact ? styles.compact : "sticky top-36 z-20 flex flex-wrap items-center gap-1 rounded-lg border border-[var(--mikke-line)] bg-white p-1 shadow-sm"}>
      {compact ? <><div className={styles.row}>
        <button type="button" title="見出し・本文" aria-label="見出し・本文" aria-expanded={menu==="type"} onClick={()=>setMenu(menu==="type"?null:"type")}><Type size={18}/></button>
        <button type="button" title="太字" aria-label="太字" onMouseDown={e=>e.preventDefault()} onClick={()=>format("bold")}><Bold size={18}/></button>
        <button type="button" title="取り消し線" aria-label="取り消し線" onMouseDown={e=>e.preventDefault()} onClick={()=>format("strikeThrough")}><Strikethrough size={18}/></button>
        <button type="button" title="リンク" aria-label="リンク" onMouseDown={e=>e.preventDefault()} onClick={()=>{setMenu(null);openLink();}}><LinkIcon size={18}/></button>
        <button type="button" title="文字の配置" aria-label="文字の配置" aria-expanded={menu==="align"} onClick={()=>setMenu(menu==="align"?null:"align")}><AlignLeft size={18}/></button>
        {onDesign && <button type="button" title="文字のデザイン" aria-label="文字のデザイン" onClick={()=>{setMenu(null);onDesign();}}><Settings2 size={18}/></button>}
      </div>
      {menu && <div className={styles.menu} role="group" aria-label={menu==="type"?"見出し・本文の選択":"文字の配置の選択"}>
        {menu==="type" ? (["2","3","paragraph"] as const).map(kind=><button key={kind} type="button" aria-pressed={kind==="paragraph"?block.type==="paragraph":block.type==="heading"&&String(block.level??2)===kind} onClick={()=>{onChange({...block,type:kind==="paragraph"?"paragraph":"heading",level:kind==="3"?3:2});setMenu(null);ref.current?.focus();}}>{kind==="2"?"見出し 大":kind==="3"?"見出し 小":"本文"}</button>) : (["left","center","right"] as const).map(align=><button key={align} type="button" aria-pressed={(block.align??"left")===align} onClick={()=>{onChange({...block,align});setMenu(null);ref.current?.focus();}}>{align==="left"?"左寄せ":align==="center"?"中央寄せ":"右寄せ"}</button>)}
      </div>}
      </> : <>

      ...
      <article className="mx-auto w-full max-w-[430px] overflow-hidden bg-white sm:my-6 sm:rounded-[28px] sm:border sm:border-black/10 sm:shadow-sm">
        ...
        <section className="relative">
          ...
          <div className="relative px-5 pb-6">
            ...
            <div className="pt-16">
              <InlineInput label="表示名" value={form.displayName} placeholder="名前" onChange={(value) => update("displayName", value)} className="text-2xl font-extrabold" />
              <InlineInput label="肩書き" value={form.role} placeholder="肩書き・活動内容（任意）" onChange={(value) => update("role", value)} className="mt-1 text-sm font-bold text-[var(--story-accent)]" />
        ...
        <EditorSection eyebrow="PHOTOS" title="写真（任意）" note="最大5枚。公開画面には「作品」「活動写真」などの固定タイトルは表示しません。">
          <div className="grid grid-cols-3 gap-2">
            ...
            {form.portfolio.length < 5 ? <FileButton label="写真を追加" className="flex aspect-square min-h-24 flex-col items-center justify-center rounded-2xl border border-dashed border-black/20 bg-black/[0.02] text-xs font-bold text-black/45 shadow-none" busy={uploading === "portfolio"} onFile={(file) => void upload(file, "portfolio")}><Plus size={21} /><span className="mt-1">写真を追加</span></FileButton> : null}
          </div>
        </EditorSection>

...
import { ArrowRight, Clock3, Globe2, LayoutTemplate, Plus, Sparkles } from "lucide-react";
...
export function PageDashboard() {
  ...
  return (
    ...
    >
      <section className="relative overflow-hidden rounded-[28px] border border-[var(--mikke-primary-border)] bg-[var(--mikke-primary)] px-5 py-7 text-white shadow-sm sm:px-8 sm:py-9">
        <div className="relative z-10 max-w-2xl">
          <p className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-bold"><Sparkles size={14} /> かんたんホームページ作成</p>
          <h2 className="mt-4 text-2xl font-extrabold leading-tight tracking-normal sm:text-4xl">選んで、触って、あなたらしく。</h2>
          <p className="mt-3 max-w-xl text-sm leading-7 text-white/75 sm:text-base">テンプレートから始めても、AIが作ったHTMLから始めても大丈夫。あとから必要な場所だけ、見たまま編集できます。</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href="/apps/page/new" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-bold text-[var(--mikke-primary)] shadow-sm"><Plus size={17} /> 新しいサイトを作る</Link>
            {sites[0] ? <Link href={`/apps/page/${sites[0].id}`} className="inline-flex items-center gap-2 rounded-xl border border-white/25 bg-white/10 px-4 py-3 text-sm font-bold text-white">編集を続ける <ArrowRight size={16} /></Link> : null}
          </div>
        </div>
        <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-[var(--mikke-accent)] opacity-25 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-20 right-1/4 h-52 w-52 rounded-full bg-white opacity-10 blur-3xl" />
      </section>
          ...
          {sites.map((site) => {
            ...
            return (
              <article key={site.id} className="overflow-hidden rounded-3xl border border-[var(--mikke-line)] bg-white shadow-sm">
                <div className="relative aspect-[16/9] overflow-hidden bg-[var(--mikke-surface-soft)] p-5" style={{ color: site.theme.textColor, backgroundColor: site.theme.backgroundColor }}>
                  <div className="mx-auto h-full max-w-sm rounded-xl border border-black/5 bg-white/80 p-5 shadow-sm">
                    <div className="h-2 w-16 rounded-full" style={{ backgroundColor: site.theme.accentColor }} />
                    <div className="mt-4 h-4 w-3/4 rounded-full" style={{ backgroundColor: site.theme.primaryColor }} />
                    <div className="mt-2 h-2 w-1/2 rounded-full bg-black/10" />
                    <div className="mt-6 grid grid-cols-2 gap-3"><div className="h-16 rounded-lg bg-black/[.04]" /><div className="h-16 rounded-lg bg-black/[.04]" /></div>
                  </div>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-[var(--mikke-line-soft)] pt-4"><span className="inline-flex items-center gap-1.5 text-xs text-[var(--mikke-muted)]"><Clock3 size={13} /> {firstPage ? `${site.documents.length}ページ` : "ページなし"}</span><Link href={`/apps/page/${site.id}`} className="inline-flex items-center gap-1.5 rounded-xl bg-[var(--mikke-primary)] px-3 py-2 text-xs font-bold text-white">編集する <ArrowRight size={14} /></Link></div>
                </div>
              </article>
            );
          })}

  ...
  return (
    <HubFrame>
      <form onSubmit={submit} className="mt-6 max-w-2xl rounded-lg border border-[var(--mikke-line)] bg-white p-5 md:p-6">
        ...
        <div className="mt-5 flex flex-wrap gap-2"><button disabled={saving || !user || Boolean(createBlock)} className="rounded-lg bg-[var(--mikke-accent)] px-5 py-3 text-sm font-bold text-white disabled:opacity-60">{saving ? "作成中..." : "Communityを作成"}</button><Link href="/community/manage" className="rounded-lg border border-[var(--mikke-line)] px-5 py-3 text-sm font-bold text-[var(--mikke-primary)]">運営一覧へ戻る</Link></div>
      </form>
    </HubFrame>
  );
}

...
import { CalendarDays, CheckCircle2, ChevronDown, ChevronUp, MessageSquare, Plus, UsersRound } from "lucide-react";
...
export function TeamWorksOperationsClientPortal() {
  ...
  async function mutate(action: () => Promise<void>, message: string) {
    ...
    try {
      ...
      setError(toErrorMessage(mutationError, "保存に失敗しました。"));
    }
  }
...
function SessionSummary({ session }: { session: OperationsClientSession }) {
  return <div className="rounded-2xl border border-[var(--mikke-line)] bg-[var(--mikke-surface-soft)] p-4"><p className="font-extrabold">{session.startTime}〜{endTime(session.startTime, session.durationMin)}</p><p className="mt-1 text-xs font-semibold text-[var(--mikke-muted)]">担当：{session.partnerName ?? "担当未定"}</p></div>;
}

function RosterEditor({ session, participants, mutate }: { session: OperationsClientSession; participants: OperationsClientPortalData["participants"]; mutate: (action: () => Promise<void>, message: string) => Promise<void> }) {
  ...
  return <div className="rounded-2xl border border-[var(--mikke-line)] bg-white p-4"><p className="mb-3 text-xs font-bold text-[var(--mikke-muted)]">出席順を確定</p><div className="space-y-2">{participants.map((participant) => { const index = selectedIds.indexOf(participant.id); return <div key={participant.id} className={`flex items-center gap-2 rounded-xl border p-3 ${index >= 0 ? "border-[var(--mikke-primary)] bg-[var(--mikke-primary-soft)]" : "border-[var(--mikke-line)] bg-white"}`}><input id={`participant-${session.id}-${participant.id}`} type="checkbox" checked={index >= 0} onChange={() => toggle(participant.id)} className="h-4 w-4" /><label htmlFor={`participant-${session.id}-${participant.id}`} className="min-w-0 flex-1 text-sm font-extrabold">{index >= 0 ? `${index + 1}. ` : ""}{participant.name}</label>{index >= 0 ? <div className="flex gap-1"><button type="button" onClick={() => move(participant.id, -1)} aria-label="上へ" className="rounded-lg p-1 text-[var(--mikke-primary)]"><ChevronUp size={17} /></button><button type="button" onClick={() => move(participant.id, 1)} aria-label="下へ" className="rounded-lg p-1 text-[var(--mikke-primary)]"><ChevronDown size={17} /></button></div> : null}</div>; })}{!participants.length ? <MikkeEmptyState title="名簿はまだありません" helper="名簿タブから対象者を登録してください。" /> : null}</div><button type="button" onClick={() => void mutate(() => saveOperationsClientSessionRoster(supabase, { projectId: session.projectId, sessionId: session.id, participantIds: selectedIds }), "出席順を確定しました。本部と担当パートナーに共有されます。")} className="mt-4 rounded-xl bg-[var(--mikke-primary)] px-4 py-2.5 text-sm font-bold text-white">出席順を保存</button></div>;
}

function ProjectRosterTab({ data, project, mutate }: { data: OperationsClientPortalData; project: OperationsClientPortalData["projects"][number]; mutate: (action: () => Promise<void>, message: string) => Promise<void> }) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [name, setName] = useState("");
  const [level, setLevel] = useState("");
  const participants = data.participants.filter((participant) => participant.projectId === project.id);
  function edit(participant?: typeof participants[number]) { setEditingId(participant?.id ?? null); setName(participant?.name ?? ""); setLevel(participant?.level ?? ""); }
  async function submit(event: FormEvent) { event.preventDefault(); await mutate(() => saveOperationsClientParticipant(supabase, { projectId: project.id, participantId: editingId ?? undefined, name, level }), editingId ? "名簿を更新しました。" : "対象者を登録しました。"); edit(); }
  return <MikkeSection title="名簿" tone="editorial"><form onSubmit={submit} className="grid gap-3 rounded-2xl border border-[var(--mikke-line)] bg-[var(--mikke-surface-soft)] p-4 sm:grid-cols-[1fr_1fr_auto]"><input value={name} onChange={(event) => setName(event.target.value)} placeholder="対象者名" required className="rounded-xl border border-[var(--mikke-line)] bg-white px-3 py-2.5 text-sm" /><input value={level} onChange={(event) => setLevel(event.target.value)} placeholder="補足（任意）" className="rounded-xl border border-[var(--mikke-line)] bg-white px-3 py-2.5 text-sm" /><button className="rounded-xl bg-[var(--mikke-primary)] px-4 py-2.5 text-sm font-bold text-white">{editingId ? "更新" : <span className="inline-flex items-center gap-1"><Plus size={15} />登録</span>}</button></form><div className="mt-4 grid gap-2 sm:grid-cols-2">{participants.map((participant) => <button key={participant.id} type="button" onClick={() => edit(participant)} className="rounded-xl border border-[var(--mikke-line)] bg-white p-3 text-left"><p className="font-extrabold">{participant.name}</p><p className="mt-1 text-xs font-semibold text-[var(--mikke-muted)]">{participant.level || "補足なし"}</p></button>)}</div>{!participants.length ? <MikkeEmptyState title="名簿はまだありません" helper="上のフォームから対象者を登録してください。" /> : null}</MikkeSection>;
}

function ProjectMessagesTab({ data, project, mutate }: { data: OperationsClientPortalData; project: OperationsClientPortalData["projects"][number]; mutate: (action: () => Promise<void>, message: string) => Promise<void> }) {

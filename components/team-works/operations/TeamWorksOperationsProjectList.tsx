...
// あゆみ指摘(2026-07-30)「プロジェクトが上、新規作成が下が良い」に合わせて並び替えた
// (Operations一覧 → Delivery一覧 → 作成2枚 → Templates → アーカイブ)。
// 運営型の作成フォームは1行も変えず、納品型カードと同じ「ボタン式」の外側を被せて畳んだだけ。
export function TeamWorksOperationsProjectList() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 作成2枚。見た目を揃えるため、運営型もボタン式の外枠を被せて畳む
          (中のフォーム・createOperationsProject呼び出しは1行も変えていない)。 */}
      <div className="grid gap-4 md:grid-cols-2">
        <section className="overflow-hidden rounded-2xl border border-[var(--mikke-line)] bg-white">
          <button
            type="button"
            onClick={() => setOpsFormOpen((current) => !current)}
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-base font-extrabold">運営型プロジェクトを立ち上げる</span>
              <span className="mt-1 block text-xs font-semibold leading-5 text-[var(--mikke-muted)]">契約期間中ずっと運営する仕事。例）教室、店舗、定期メンテナンス契約</span>
            </span>
            {opsFormOpen ? <ChevronUp size={16} className="mt-1 shrink-0 text-[var(--mikke-muted)]" /> : <ChevronDown size={16} className="mt-1 shrink-0 text-[var(--mikke-muted)]" />}
          </button>
          {opsFormOpen ? (
            <div className="border-t border-[var(--mikke-line-soft)] p-4 sm:p-5">
              <form onSubmit={submit} className="grid gap-3 md:grid-cols-2 md:items-end">
                ...
                {error ? <p role="alert" className="rounded-xl border border-[var(--tw-action)] px-3 py-2 text-xs font-bold text-[var(--tw-action)] md:col-span-2">{error}</p> : null}

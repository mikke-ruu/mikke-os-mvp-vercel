...
export function AcademyListTools({ label, query, onQuery, filter, onFilter, options, count }: {
  ...
  return <section aria-label={`${label}の検索と絞り込み`} className="space-y-2 text-[var(--mikke-text)]">
    ...
    <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">{options.map(option => <button key={option.value} type="button" aria-pressed={filter === option.value} onClick={() => onFilter(option.value)} className={`min-h-10 rounded-lg border px-2 text-xs font-medium sm:px-3 sm:text-sm ${filter === option.value ? "border-[var(--mikke-primary)] font-bold" : "border-[var(--mikke-line)]"}`}>{option.label} {option.count}件</button>)}</div>

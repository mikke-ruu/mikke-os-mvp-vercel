...
function Connected(props: Props) {
  ...
  return <AcademyFirstPublicationPanel identityKey={userId} state={status} access={load.access} course={course} allowedActions={allowedActions} onRefresh={async()=>{await onReloadCourse();await refresh();}} onAction={async action=>{
    if(action==="publish")await createFirstPublicationRpc(supabase)(headquartersId,{action,courseId:course.id,quoteId:status.quoteId,confirmed:true});

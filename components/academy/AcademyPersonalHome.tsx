...
export function AcademyPersonalHome({ name, view, records, learnerApps, courses, canSwitch, onSwitch }: {
  ...
  return <div className={styles.root}>
    ...
    <nav className={styles.menu} aria-label="その他のメニュー"><Link href="/community">Community</Link>{teaching && active.length > 0 && <><Link href={href("/academy/portal/kits")}>教材注文</Link><Link href={href("/academy/portal/study?view=learner")}>自分の受講教材</Link></>}</nav>
  </div>;
}

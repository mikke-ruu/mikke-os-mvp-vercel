...
import styles from "./offering-editor.module.css";

const field = styles.field;
const action = styles.action;
...
export function OfferingEditor({ initial, initialInput, courses, onSave, onArchive }: {
  initial?: AcademyOffering; initialInput?: OfferingInput; courses: AcademyCourse[];
  onSave: (input: OfferingInput) => Promise<AcademyOffering>;
  onArchive?: () => Promise<AcademyOffering>;
}) {
  const [form, setForm] = useState<OfferingInput>(() => initial ? offeringInput(initial) : initialInput ? structuredClone(initialInput) : blankOfferingInput());
  ...
  function go(next: number) { setStep(next); top.current?.scrollIntoView({ block: "start" }); top.current?.focus({ preventScroll: true }); }
  ...
  async function save() {
    ...
    if (form.status === "published" && selected.some(course => !course.is_published)) { setError("公開前に講座の公開状態を確認してください。"); setStep(2); return; }
  ...
  return <div ref={top} tabIndex={-1} className={styles.flow}>
    <div className={styles.toolbar}><Link className={action} href={toCurrentAcademyContextHref("/academy/offerings")} onClick={event => { if (dirty && !window.confirm("保存していない変更があります。一覧に戻りますか？")) event.preventDefault(); }}>× 閉じる（募集一覧へ）</Link><span className="text-xs">{dirty ? "未保存" : initial ? "保存済み" : "新しい募集"}</span></div>
    <header className="mb-5"><p className="text-[11px] tracking-[.14em]">MY ACADEMY</p><h1 className="mt-1 text-2xl font-bold">{initial ? "募集を編集" : "募集をつくる"}</h1></header>
    <div className={styles.tools}><AcademyRolePreview role="learner" title={form.title} courseNames={selected.map(course => course.name)} price={form.purchase_mode === "staged" ? form.course_ids.reduce((sum, id) => sum + (form.stage_prices[id] ?? 0), 0) : form.price} /></div>
    ...
    <nav aria-label="募集作成の手順" className={styles.steps}>{["1–3 募集・販売の設定", "4 ページを編集", "5 公開設定"].map((label, index) => <button key={label} type="button" disabled={saving} aria-current={step === index ? "step" : undefined} onClick={() => go(index)}>{label}</button>)}</nav>
    <fieldset disabled={saving} className="min-w-0 space-y-5">
      {step === 2 && selected.some(course => !course.is_published) && <section className="space-y-2 border-l-4 border-amber-400 bg-amber-50 p-3 text-sm"><h3 className="font-bold">公開前に確認する講座</h3><p>新しい講座は下書きで保存されます。講座の編集画面で公開状態を設定してください。変更後はこの募集を下書きで保存し、再読み込みしてください。</p>{selected.filter(course => !course.is_published).map(course => <p key={course.id}><Link className="underline" target="_blank" rel="noopener noreferrer" href={toCurrentAcademyContextHref(`/academy/courses/${course.id}`)}>{course.name}を確認</Link>（下書き）</p>)}</section>}
      {step === 0 && <div className={styles.setup}>
        <section className={styles.group}><h2><span>1</span>募集内容</h2><label className="block text-sm font-bold">募集名<input className={field} value={form.title} maxLength={200} onChange={event => change("title", event.target.value)} /></label><label className="block text-sm font-bold">種類<select className={field} value={form.kind} onChange={event => change("kind", event.target.value as OfferingInput["kind"])}>{offeringKinds.map(kind => <option key={kind}>{kind}</option>)}</select></label>
      ...
      {form.purchase_mode === "staged" && <p className="text-sm">講座の順番は選択した順です。初回 ¥{(form.stage_prices[form.course_ids[0]] ?? 0).toLocaleString()} ／ 全講座合計 ¥{form.course_ids.reduce((total, id) => total + (form.stage_prices[id] ?? 0), 0).toLocaleString()}。前の講座の修了後に次の講座へ進みます。</p>}
          {(form.kind === "月額レッスン") && <p role="status" className="text-sm">月額レッスンはまだ受付に接続していません。下書きとして保存できます。</p>}
        </section>
        <section className={styles.group}><h2><span>3</span>受講内容</h2><label className="block text-sm font-bold">講座の修了確認<select className={field} value={form.completion_mode} onChange={event => change("completion_mode", event.target.value as OfferingInput["completion_mode"])}><option value="learner">受講者が修了を記録</option><option value="hq">本部が修了を確認</option></select></label><p className="text-sm">教材は各講座で編集します。募集ページには教材本文を公開しません。</p>{selected.map(course => <div key={course.id} className="flex flex-wrap items-center justify-between gap-2 border-b py-2"><span className="text-sm">{course.name}</span><div className="flex gap-3 text-sm"><Link target="_blank" rel="noopener noreferrer" href={toCurrentAcademyContextHref(`/academy/courses/${course.id}/instructor-page?audience=learner`)}>レッスン教材</Link><Link target="_blank" rel="noopener noreferrer" href={toCurrentAcademyContextHref(`/academy/courses/${course.id}/instructor-page`)}>講師マニュアル</Link></div></div>)}</section>
      </div>}
      ...
      {step === 2 && <section className={`${styles.setup} ${styles.group}`}><h2><span>5</span>公開設定</h2><p className="text-sm">公開して保存すると、募集ページのURLから内容を見られるようになります。</p><label className="block text-sm font-bold">公開状態<select className={field} value={form.status} onChange={event => change("status", event.target.value as OfferingInput["status"])}><option value="draft">下書き</option><option value="published">公開する</option><option value="archived">受付を終了（アーカイブ）</option></select></label><dl className="grid grid-cols-[auto_1fr] gap-3 text-sm"><dt>募集名</dt><dd>{form.title || "未入力"}</dd><dt>講座</dt><dd>{selected.map(course => course.name).join(" ／ ") || "未選択"}</dd><dt>募集価格</dt><dd>{Number.isFinite(form.price) ? `¥${form.price.toLocaleString()}` : "未入力"}</dd></dl>{initial?.status === "published" && <Link href={`/academy/o/${initial.id}`} target="_blank" rel="noopener noreferrer" className={action}>公開ページを確認</Link>}{onArchive && initial?.status !== "archived" && <button type="button" className={action} onClick={archive}>募集をアーカイブ</button>}</section>}
    </fieldset>
    <div className={styles.flowActions}>{step > 0 && <button type="button" className={action} disabled={saving} onClick={() => go(step - 1)}>戻る</button>}{step < 2 && <button type="button" className={action} disabled={saving} onClick={() => go(step + 1)}>{step === 0 ? "次へ：ページを編集" : "次へ：公開設定"}</button>}</div>
    <div className={styles.saveBar}><button type="button" disabled={saving} onClick={save} className={action}>{saving ? "保存中…" : "保存する"}</button></div>
  </div>;
}

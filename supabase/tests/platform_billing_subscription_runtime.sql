...
foreach f in array array['public.platform_billing_verified_subscription_activate(uuid,text,text,text,text,text,bigint,text,timestamptz)'::regprocedure,'public.platform_billing_subscription_event_apply(text,text,text,text,text,timestamptz,timestamptz,boolean,timestamptz)'::regprocedure,'public.platform_billing_status_get(uuid,text,uuid)'::regprocedure,'public.platform_billing_portal_context(uuid,text,uuid)'::regprocedure,'public.platform_billing_academy_new_paid_consume(uuid,uuid)'::regprocedure,'public.platform_billing_academy_existing_paid_consume(uuid,uuid)'::regprocedure]loop
...
select pg_temp.sub_assert(public.platform_billing_academy_new_paid_consume('a9040000-0000-4000-8000-000000000001','a9040000-0000-4000-8000-000000000099')->>'verified'='true','Academy new paid verifier');

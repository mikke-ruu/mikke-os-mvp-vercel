-- Corrective: verified against the production function on 2026-09-09.
-- No guard bypass, permission widening, or change to entitlement-first locking.
...
declare
  ...
  v_consumed integer;
  v_resource_id uuid := pg_catalog.gen_random_uuid();
begin
  ...
  -- Bind the new UUID before parent INSERT: its AFTER INSERT safety initializer
  -- already invokes child-table owner guards. This row is not visible to other
  -- transactions before commit. Any parent/initializer/child failure rolls back
  -- this consumption as well; no guard is bypassed or disabled.
  update platform_billing_private.creation_entitlements e
  set status = 'consumed', resource_id = v_resource_id,
  ...
  insert into public.community_communities (id, slug, name, description, join_mode, status, owner_user_id)
  values (v_resource_id, v_slug, trim(p_name), nullif(trim(p_description), ''), 'open_free', 'active', v_actor)

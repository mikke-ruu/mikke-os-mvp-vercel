...
grant select on table public.community_academy_access_invitations to authenticated;
grant select on table public.community_academy_entitlement_claims to authenticated;
...
create policy "invitees and staff can read Academy access invitations"
on public.community_academy_access_invitations for select
to authenticated
using (user_id = (select auth.uid()) or community_private.is_staff(community_id));

create policy "claimants and staff can read Academy entitlement claims"
on public.community_academy_entitlement_claims for select
to authenticated
using (user_id = (select auth.uid()) or community_private.is_staff(community_id));
...
begin
  select
    count(*),
    min(claim.starts_at),
    case when bool_or(claim.ends_at is null) then null else max(claim.ends_at) end
  ...
  where claim.community_id = p_community_id
    ...
    and claim.starts_at <= now()
    and (claim.ends_at is null or claim.ends_at > now());
...
begin
  ...
  if char_length(trim(coalesce(p_source_reference, ''))) not between 1 and 200 then
  ...
  insert into public.community_academy_access_invitations (
    ...
    trim(p_source_reference), p_academy_role, 'pending', p_starts_at, p_ends_at, p_expires_at
  )
...
begin
  ...
  insert into public.community_join_applications (
    ...
    v_invitation.community_id, v_user_id, trim(p_display_name), nullif(trim(p_legal_name), ''),
    v_email, nullif(trim(p_phone), ''), nullif(trim(p_join_reason), ''),
    'approved', null, 'Approved by accepted Academy access invitation', now(), now()
  )
  ...
  values (v_invitation.community_id, v_user_id, trim(p_display_name))
  on conflict (community_id, user_id)
...
begin
  ...
  where claim.mapping_id = p_mapping_id
    and claim.user_id = p_user_id
    and claim.source_reference = trim(p_source_reference)
  for update;
...
begin
  ...
  if char_length(trim(coalesce(p_payer_name, ''))) not between 1 and 120 then raise exception 'Payer name is required'; end if;
  ...
  if exists (
    ...
    where academy_claim.community_id = p_community_id
      ...
      and academy_claim.starts_at <= now()
      and (academy_claim.ends_at is null or academy_claim.ends_at > now())
  ) then
  ...
  insert into public.community_payment_claims (
    ...
    p_community_id, p_plan_id, v_user_id, trim(p_payer_name),
...
begin
  if new.status = 'approved' and old.status is distinct from 'approved' then
    update public.community_memberships
    set access_scope = 'community', status = 'active', updated_at = now()

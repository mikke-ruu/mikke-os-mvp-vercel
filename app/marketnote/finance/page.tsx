...
function MarketFinanceContent() {
  ...
  async function load() {
    ...
    setDrafts((current) => hydrateDrafts(current, nextEvents, nextRecords));
  ...
  function addDraft(event: MarketEvent, recordType: "revenue" | "expense") {
    const title = recordType === "revenue" ? "売上" : "経費";
    setDrafts((current) => ({
      ...current,
      [event.id]: [
        ...(current[event.id] ?? []),
      ...
      <div className="-mx-1 pb-2">
        <header className="mb-5 pt-2">
          <div className="flex items-center justify-between">
            <h1 className="text-3xl font-semibold tracking-normal text-[#1f1b18]">収支</h1>
          ...
          <div className="mt-6 grid grid-cols-[48px_1fr_48px] items-center gap-3">
            ...
            <h2 className="text-center text-2xl font-semibold tracking-normal text-[#1f1b18]">
        ...
        <section className="mt-4 space-y-3">
          {rows.length > 0 ? rows.map((row) => {
            ...
            return (
              <article key={row.event.id} className="rounded-[18px] border border-[#e7e1dc] bg-white shadow-[0_5px_16px_rgba(45,33,22,0.045)]">
                <button
                  type="button"
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <h3 className="truncate text-lg font-extrabold tracking-normal text-[#1f1b18]">{row.event.title}</h3>
                      <div className="mt-2 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-xs font-bold text-[#5f5a55]">
                        <span className="inline-flex items-center gap-1"><Clock3 size={14} />{formatMonthDay(row.event.event_date)}</span>
                        <span className="inline-flex min-w-0 items-center gap-1"><MapPin size={14} /><span className="truncate">{row.event.venue_name || row.event.area || "会場未設定"}</span></span>
                      </div>
                    </div>
                    <ChevronDown size={19} className={`mt-1 shrink-0 text-[#1f1b18] transition ${open ? "rotate-180" : ""}`} />
                  </div>

                  <div className="mt-4 border-t border-[#f1ece7] pt-3">
                    <FinanceSummaryGrid revenue={row.revenue} expense={row.expense} profit={row.profit} compact />
                    <p className="mt-3 text-right text-xs font-extrabold text-[#f46a14]">収支を編集 〉</p>
                  </div>
                </button>

                {open ? (
                  <div className="border-t border-[#f1ece7] px-4 pb-4 pt-3">
                    <h4 className="mb-3 text-sm font-extrabold text-[#1f1b18]">収支詳細</h4>
                    <FinanceDraftSection
                      title="売上内訳"
                      actionLabel="売上を追加"
                      recordType="revenue"
                    ...
                    <FinanceDraftSection
                      ...
                      onRemove={(draft) => removeDraft(row.event.id, draft)}
                    />
                    <div className="mt-4 flex items-end justify-between border-t border-[#f1ece7] pt-3">
                      <span className="text-base font-extrabold text-[#1f1b18]">利益</span>
                      <span className="text-2xl font-semibold tracking-normal text-[#16833b]">{formatYen(getDraftTotals(eventDrafts).profit)}</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => saveEventFinance(row.event)}
                      disabled={savingEventId === row.event.id}
                      className="mt-4 w-full rounded-xl bg-[#ff5a1f] px-4 py-3 text-base font-extrabold text-white shadow-[0_8px_18px_rgba(255,90,31,0.16)] disabled:opacity-50"
                    >
...
function MonthlySummary({ totals }: { totals: { revenue: number; expense: number; profit: number } }) {
  return (
    <section className="rounded-[18px] border border-[#e7e1dc] bg-white px-3 py-4 shadow-[0_5px_16px_rgba(45,33,22,0.045)]">
...
function FinanceSummaryGrid({
  ...
  return (
    <div className="grid grid-cols-[1fr_1px_1fr_1px_1fr] items-center">
      ...
      <span className={compact ? "h-9 bg-[#eee9e4]" : "h-12 bg-[#eee9e4]"} />
...
function FinanceAmount({
  ...
  return (
    <div className="text-center">
      <p className={`${compact ? "text-xs" : "text-sm"} font-bold ${profit ? "text-[#16833b]" : "text-[#3b3530]"}`}>{label}</p>
      <p className={`${compact ? "mt-1 text-base" : "mt-1.5 text-2xl"} font-semibold tracking-normal ${profit ? "text-[#16833b]" : muted ? "text-[#5f5a55]" : "text-[#1f1b18]"}`}>{formatYen(value)}</p>
    </div>
  );
}
  ...
  return (
    <div className="mt-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button type="button" className="text-xs font-extrabold text-[#ff5a1f]">カテゴリ設定</button>
      </div>
      <div className="space-y-2">
        {drafts.map((draft) => (
          <div key={draft.id} className="grid grid-cols-[1fr_104px_28px] items-center gap-2 rounded-xl border border-[#e7e1dc] bg-white px-2.5 py-2">
            <input
              value={draft.title}
              ...
              className="min-w-0 bg-transparent text-sm font-bold text-[#1f1b18] outline-none placeholder:text-[#b4aaa2]"
            />
            <MoneyInput value={draft.amount} onChange={(value) => onChange(draft.id, { amount: value, occurredAt: draft.occurredAt || event.event_date })} />
            <button type="button" onClick={() => onRemove(draft)} className="grid h-7 w-7 place-items-center rounded-full text-[#8a817a]" aria-label="削除">
      ...
      <button type="button" onClick={onAdd} className="mt-2 inline-flex items-center gap-1 rounded-full px-1 py-1 text-sm font-extrabold text-[#ff5a1f]">
        <Plus size={16} strokeWidth={1.8} />
        {actionLabel}
      </button>
    </div>
...
function MoneyInput({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <div className="grid h-9 grid-cols-[22px_1fr] overflow-hidden rounded-lg bg-[#fbfaf8]">
      ...
      <input
        ...
        pattern="[0-9]*"
        placeholder="0"
        className="min-w-0 bg-transparent pr-2 text-right text-sm font-extrabold text-[#1f1b18] outline-none"
      />
    </div>
  );
}
...
function hydrateDrafts(
  current: Record<string, FinanceDraft[]>,
  events: MarketEvent[],
  records: MarketFinancialRecord[]
) {
  ...
  for (const event of events) {
    const eventRecords = records.filter((record) => record.market_event_id === event.id);
    const currentNewRows = (current[event.id] ?? []).filter((draft) => !draft.persistedId);
    next[event.id] = [
      ...eventRecords.map((record) => recordToDraft(record)),
      ...currentNewRows
    ];
  }

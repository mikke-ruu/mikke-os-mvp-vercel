...
function CoursesContent() {
  ...
  if (loading) return <p className="py-10 text-center text-sm text-[var(--mikke-text)]">読み込み中…</p>;
  ...
  if (!hq) {
    return (
      <div className="space-y-3 rounded-lg border border-[var(--mikke-line)] bg-white p-5 text-center">
        ...
        <p className="text-xs text-[var(--mikke-text)]">契約確認後、Academyのホームから本部を作成してください。</p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="text-base font-bold text-[var(--mikke-text)]">講座一覧</h2>
        </div>
        {createAccess?.allowed ? (
          ...
          <span aria-disabled="true" className="flex items-center gap-1 rounded-lg bg-[var(--mikke-line)] px-3 py-2 text-xs font-bold text-[var(--mikke-text)]">
            <Plus size={16} /> 新規
          </span>
        )}
      </div>
      ...
      {!createAccess?.allowed && createAccess?.reason ? (
        <p role="status" className="rounded-lg border border-[var(--mikke-line)] bg-[var(--mikke-surface-soft)] px-4 py-3 text-xs leading-5 text-[var(--mikke-text)]">
          {createAccess.reason}
        </p>
      ) : null}
      ...
      {courses.length === 0 ? (
        <div className="rounded-lg border border-dashed border-[var(--mikke-line)] bg-white p-8 text-center">
          ...
          <p className="mt-2 text-sm leading-7 text-[var(--mikke-text)]">教えたいことの名前と受講料を決めるだけ。<br />紹介文や写真は後から追加できます。</p>
          {createAccess?.allowed ? (
              ...
              <div className="space-y-2 p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="break-words text-base font-bold text-[var(--mikke-text)]">{course.name}</span>
                    </div>
                    <p className="mt-1 text-xs text-[var(--mikke-text)]">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[format, features.certification ? "認定講座" : null].filter(Boolean).map((label) => (
                    <span key={label} className="rounded-lg bg-[var(--mikke-surface-soft)] px-2 py-1 text-xs text-[var(--mikke-text)]">{label}</span>
                  ))}
                </div>

                <p className="text-sm leading-5 text-[var(--mikke-text)]">{course.subtitle || (course.is_published ? "公開中の内容を確認・編集できます。" : "下書きを保存済みです。続きをつくりましょう。")}</p>

                <div className="grid grid-cols-2 gap-2 pt-1 [&_a]:min-h-11">
                  <Link
                  ...
                  >
                    <LayoutTemplate size={14} /> 講座ページを編集
                  </Link>
                </div>
                <section><div className="grid grid-cols-2 gap-2 [&_a]:min-h-11">
                  <Link
                    href={`/academy/c/${course.id}`}
                    target="_blank"
                    className="flex items-center justify-center gap-1.5 rounded-lg border border-[var(--mikke-line)] px-2 py-2 text-xs font-bold text-[var(--mikke-text)]"
                  >
                    <Eye size={14} /> 講座ページを見る
                  </Link>
                  <Link
                    href={toCurrentAcademyContextHref(`/academy/courses/${course.id}/instructor-page`)}
                    className="flex items-center justify-center gap-1.5 rounded-lg border border-[var(--mikke-line)] px-2 py-2 text-xs font-bold text-[var(--mikke-text)]"
                  >
                    <GraduationCap size={14} /> 講師マニュアルページ
                  </Link>
                  <Link

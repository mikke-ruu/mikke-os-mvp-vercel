...
import { loadImplementationCenter, updateImplementationItemStatus, type ImplementationConversation, type ImplementationGate, type ImplementationItem, type ImplementationMessage, type ImplementationProject } from "@/lib/implementation-center";
...
export default function ImplementationCenterPage() {
  ...
  async function load(silent = false) {
    ...
    try { const data = await loadImplementationCenter(); setProjects(data.projects); setGates(data.gates); setItems(data.items); setConversations(data.conversations); setMessages(data.messages); }
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    <header className="rounded-3xl border border-[var(--mikke-line)] bg-[linear-gradient(135deg,#132744_0%,#1d3b61_65%,#285377_100%)] p-5 text-white shadow-sm md:p-7">
      <div className="flex flex-wrap items-start justify-between gap-5"><div className="max-w-3xl"><p className="text-xs font-bold tracking-[0.18em] text-blue-200">MIKKEOS IMPLEMENTATION CENTER</p><h1 className="mt-3 text-2xl font-bold md:text-3xl">実装センター</h1><p className="mt-3 text-sm leading-6 text-blue-100">アプリごとにCodexと相談を続け、アイデアが決まったら同じ会話から実装へ移せます。進捗・確認依頼・結果もこの画面へ戻ります。</p></div><a href="#app-consultation-room" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-bold text-[#183353] shadow-sm"><MessageSquarePlus size={17} />相談室を開く</a></div>
      <div className="mt-6 grid grid-cols-2 gap-2 md:grid-cols-4">{[["実装中", projects.filter((p) => p.status === "active").length], ["判断待ち", items.filter((i) => i.status === "waiting_user").length + conversations.filter((conversation) => conversation.status === "waiting_user").length], ["公開中", projects.filter((p) => p.public_state === "public").length], ["会話中", conversations.filter((conversation) => conversation.status !== "archived").length]].map(([label, value]) => <div key={String(label)} className="rounded-xl bg-white/10 px-4 py-3"><p className="text-[11px] text-blue-200">{label}</p><p className="mt-1 text-xl font-bold">{value}</p></div>)}</div>
    </header>
    ...
    <div id="app-consultation-room" className="scroll-mt-5"><ImplementationConversationPanel project={selectedProject} conversations={conversations} messages={messages} onChanged={() => load(true)} /></div>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="space-y-5">
        ...
        <p className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-[11px] leading-5 text-blue-800"><ShieldCheck size={15} className="mb-1" />画面で交わしたメッセージだけをHQ内に非公開保存します。Codexの内部思考・トークン・認証情報は保存しません。</p>
      </aside>
    </div>
  </div>;
}

...
import { ArrowDown, ArrowLeft, ArrowUp, Eye, Plus, RotateCcw, Save, Star, Tag } from "lucide-react";
...
const renameWarning = [
  "この項目はすでに収支データで使用されています。",
  "",
  "項目名を変更すると、過去にこの項目で登録した収支データの表示名も変更されます。",
  "",
  "意味が変わる場合は、名前を変更せず、新しい項目を追加してください。",
  "",
  "変更してもよろしいですか？"
].join("\n");
...
function FinanceCategoriesContent() {
  ...
  const title = activeType === "revenue" ? "売上項目" : "経費項目";
  ...
  function resetCategories() {
    ...
    setMessage("初期項目に戻しました。保存すると収支ページに反映されます。");
  }

  function save() {
    saveFinanceCategorySettings(settings);
    setMessage("収支項目を保存しました。収支ページの項目候補に反映されます。");
  }

  return (
    <AppShell title="収支項目" hideHeader hideBottomNav>
      <div className="pb-5">
        <header className="mb-4 grid grid-cols-[40px_1fr_40px] items-center pt-1">
          ...
          <h1 className="text-center text-xl font-semibold tracking-normal text-[#1f1b18]">収支項目</h1>
          <span />
        </header>
        ...
        <p className="mb-3 rounded-2xl border border-[#eee9e4] bg-white px-4 py-3 text-xs font-bold leading-5 text-[#6f6862] shadow-[0_3px_12px_rgba(45,33,22,0.035)]">
          収支ページで使う売上項目と経費項目を管理します。☆を付けた項目は目印として表示し、候補の順番は矢印で並び替えます。
        </p>
        ...
        <div className="mb-3 grid grid-cols-2 gap-2 rounded-2xl border border-[#eee9e4] bg-white p-1.5 shadow-[0_3px_12px_rgba(45,33,22,0.035)]">
          <CategoryTab label="売上項目" active={activeType === "revenue"} onClick={() => setActiveType("revenue")} />
          <CategoryTab label="経費項目" active={activeType === "expense"} onClick={() => setActiveType("expense")} />
        </div>
          ...
          <div className="border-b border-[#f1ece7] p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-bold text-[#8a817a]">{activeItems.length}項目</p>
              </div>
        ...
        <section className="mt-3 rounded-2xl border border-[#eee9e4] bg-white p-3.5 shadow-[0_3px_12px_rgba(45,33,22,0.035)]">
          ...
          <div className="mt-2 space-y-1.5 text-xs font-bold leading-5 text-[#6f6862]">
            <p>項目は完全削除せず、使わなくなったものは非表示にします。</p>
            <p>項目名を変えるときは、意味が変わらないか確認してください。意味が変わる場合は新しい項目を追加します。</p>
            <p>保存した並び順は、収支ページの項目候補にも反映されます。</p>
          </div>
        </section>

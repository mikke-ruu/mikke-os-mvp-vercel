...
const academyHidden = projectMikkeMenuPreferences(
  ["marketnote", "academy", "community"],
  [{ app_key: "academy", sort_order: 0, is_hidden: true }]
);
assert.deepEqual(academyHidden.ownedAppKeys, ["academy", "marketnote", "community"]);
assert.deepEqual(academyHidden.hiddenOwnedAppKeys, ["academy"]);
assert.deepEqual(academyHidden.visibleOwnedAppKeys, ["marketnote", "community"]);
assert.deepEqual(
  projectMikkeMenuPreferences(["academy", "marketnote"], []).visibleOwnedAppKeys,
  ["marketnote", "academy"]
);
assert.deepEqual(
  projectMikkeMenuPreferences(["story"], [{ app_key: "academy", sort_order: 0, is_hidden: false }]).ownedAppKeys,
  ["story"]
);
assert.deepEqual(menuPreferenceReplaceArguments(menuPreferenceRowsFromDraft({
  orderedAppKeys: ["academy"], hiddenAppKeys: ["academy"]
})), { p_items: [{ app_key: "academy", sort_order: 0, is_hidden: true }] });
console.log("academy menu preference preservation: ok");

...
function fixture(amount=10000,plan='medium'){
 ...
 const quote={price_id:'proof-price-uuid',snapshot_id:'snapshot-uuid',amount_yen:amount,plan_key:plan,period_start:job.period_start,period_end:job.period_end};
 ...
 const request=async(url,init)=>{calls.push({url,init});const path=new URL(url).pathname.replace('/v1/','');let value;
   if(path==='prices/price_New')value={id:'price_New',active:true,livemode:false,currency:'jpy',unit_amount:unitAmount,billing_scheme:'per_unit',transform_quantity:null,recurring:{interval:'month',interval_count:1,usage_type:'licensed'}};
   else if(path==='subscriptions/sub_A'){if(init.method==='POST'){assert.equal(init.body.get('proration_behavior'),'none');assert.equal(init.body.get('pause_collection[behavior]'),'keep_as_draft');assert.equal(Number(init.body.get('items[0][quantity]')),quantity);sub.items.data[0].price={id:'price_New',currency:'jpy',unit_amount:unitAmount};sub.items.data[0].quantity=quantity;}value=sub;}
   else if(path==='invoices')value={data:[invoice],has_more:false};
   else if(path==='invoices/in_A')value=invoice;
   else if(path==='invoices/in_A/lines/il_A'){assert.equal(init.body.get('price'),'price_New');assert.equal(Number(init.body.get('quantity')),quantity);if(!skipLine){invoice.lines.data[0].price={id:'price_New'};invoice.lines.data[0].quantity=quantity;invoice.lines.data[0].amount=invoice.total=invoice.amount_due=amount;}value=invoice.lines.data[0];}
   else if(path==='invoices/in_A/finalize'){assert.equal(init.body.get('auto_advance'),'false');invoice.status='open';value=invoice;}
   else if(path==='invoices/in_A/pay'){assert.equal(init.body.get('off_session'),'true');invoice.status='paid';invoice.amount_paid=amount;invoice.amount_remaining=0;invoice.status_transitions={paid_at:clock/1000};value=invoice;}
 ...
 return{job,sub,invoice,calls,store,quote,run:()=>processRenewalJob(job,{stripe,store,priceIds:{[plan]:'price_New'},now:()=>clock},new AbortController().signal),noQuote:()=>noQuote=true,disallow:()=>allowed=false,skipLine:()=>skipLine=true,clock:v=>clock=v};
}
...
await test('201 and 250 instructor snapshots use 100 yen licensed quantity without proration',async()=>{for(const count of [201,250]){const f=fixture(count*100,'variable');assert.equal((await f.run()).amount_yen,count*100);assert.equal(f.sub.items.data[0].quantity,count);assert.equal(f.invoice.lines.data[0].quantity,count);}});
await test('variable quantity changes even if the Stripe price ID stays the same',async()=>{const f=fixture(25000,'variable');f.sub.items.data[0]={id:'si_A',price:{id:'price_New',currency:'jpy',unit_amount:100},quantity:201};assert.equal((await f.run()).outcome,'paid');assert.equal(f.sub.items.data[0].quantity,250);});
await test('invalid variable snapshot total stops before provider calls',async()=>{const f=fixture(20101,'variable');await assert.rejects(f.run(),/INVALID_VARIABLE_AMOUNT/);assert.equal(f.calls.length,0);});

...
for(const word of ['講座一覧','AcademyCatalogList','duplicate=','courseId=','getMyAcademyCourseCreationAccess'])assert.ok(list.includes(word),word);
const catalog=read('components/academy/AcademyCatalogList.tsx');
for(const word of ['複製','募集をつくる','編集する','aria-pressed'])assert.ok(catalog.includes(word),word);

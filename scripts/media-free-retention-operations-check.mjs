...
const container="mikke-media-retention-replay-20260910";
...
const files=[
  ...
  "../supabase/migrations/20260910090000_media_free_retention_operations.sql",
  "../supabase/tests/media_free_retention_operations.sql"
];
function command(args,input,allowFailure=false){
  const result=spawnSync(docker,args,{input,encoding:"utf8",timeout:300000,maxBuffer:40*1024*1024});
  if(!allowFailure&&result.status!==0){
    throw Error(result.stderr||result.stdout||result.error?.message||`docker exit ${result.status}`);
  }
  return result;
}
function psql(sql){
...
function snapshot(){
  return psql(`select json_build_object(
    'mediaRelations',(select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
      where c.relkind in ('r','p') and c.relname like 'media_%'),
...
assert.equal(
  createHash("sha256").update(baseline).digest("hex").toUpperCase(),
  "521BF5A61EB8FE572011526FAA469A679328F581E3BC291191AEF18379C97299"
);
...
try{
  ...
  for(let i=0;i<40;i+=1){
    if(command(["exec",container,"pg_isready","-U","postgres"],undefined,true).status===0){ready=true;break;}
  ...
  assert.ok(output.split(/\r?\n/).includes("media_free_retention_operations_test_ok"));
  assert.equal(snapshot(),before,"Rollback residue detected");
  console.log(JSON.stringify({
    scope:"isolated-postgres-17.6",
    baselineSha256:createHash("sha256").update(baseline).digest("hex").toUpperCase(),
    migrationSha256:createHash("sha256").update(sqlFiles[3]).digest("hex"),
    testSha256:createHash("sha256").update(sqlFiles[4]).digest("hex"),
    sentinel:"media_free_retention_operations_test_ok",
    rollback:true,
    residue:0,
    cleanup:"container and anonymous volume removed"
  }));
}finally{
  if(created){
    const removed=command(["rm","--force","--volumes",container],undefined,true);
    if(removed.status!==0)throw Error(removed.stderr||removed.stdout);
  }
}

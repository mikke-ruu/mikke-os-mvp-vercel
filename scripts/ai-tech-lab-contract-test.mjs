...
const checks = [
  ...
  ["service role can run ingestion without delete", /grant select, insert, update on[\s\S]+to service_role/.test(contents.migration) && !/grant select, insert, update, delete on[\s\S]+to service_role/.test(contents.migration)],

...
assert.ok(/if \(!lease\.isCurrent\(\)\) return;\r?\n\s+router\.replace/.test(hub));
...
assert.ok(app.includes('actorClient={actorClient} actorIsCurrent={current}'));
assert.ok(app.includes('updateCommunityMembershipPlan(actorClient'));
assert.ok(app.includes('recordCommunityManualPayment(actorClient'));
assert.ok(app.includes('createCommunityResourceFile(actorClient'));
assert.ok(app.includes('if (!actorIsCurrent()) return;'));
assert.ok(app.includes('window.sessionStorage.setItem(manualRequestStorageKey'));

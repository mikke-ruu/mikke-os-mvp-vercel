...
function fixture(){
 ...
 const a={attempt_id:attempt,started_at:new Date(end).toISOString(),owner_user_id:owner,headquarters_id:hq,quote_id:quote,amount_yen:3300,policy_version:'fixture',provider_customer_id:'cus_A',checkout_session_id:'cs_test_A',setup_intent_id:'seti_A',payment_method_id:'pm_A',status:'verified'};
 ...
 const request=async(url,init)=>{
   ...
   else if(path==='prices/price_A')value={id:'price_A',active:true,currency:'jpy',unit_amount:3300,recurring:{interval:'month',interval_count:1}};
   ...
   else if(path==='invoiceitems'){assert.equal(body.get('invoice'),'in_A');item={id:'ii_A',invoice:'in_A',customer:'cus_A',amount:3300,currency:'jpy'};invoice.total=invoice.amount_due=amountMismatch?3301:3300;value=item;}
   ...
   else if(path==='invoices/in_A/pay'){assert.equal(body.get('off_session'),'true');if(!paymentPending){invoice.status='paid';invoice.amount_paid=3300;invoice.amount_remaining=0;invoice.status_transitions={paid_at:clock/1000};}value=invoice;}
   else if(path==='invoices/in_A')value=invoice;
   else if(path==='subscriptions'){assert.equal(body.get('proration_behavior'),'none');assert.equal(body.has('trial_end'),false);sub={id:'sub_A',customer:'cus_A',status:'active',metadata:meta,livemode:false,billing_cycle_anchor:Number(body.get('billing_cycle_anchor')),items:{data:[{price:{id:'price_A'},quantity:1,current_period_end:Number(body.get('billing_cycle_anchor'))}]}};value=sub;}
 ...
 const job={event_key:'event_A',lease_token:'lease_A',kind:'start_paid',trial_ends_at:new Date(end).toISOString(),amount_yen:3300,plan_key:'small',proof:a};
 return{stripe,store,job,calls,checkpoints,a,run:()=>processBillingJob(job,{stripe,store,now:()=>clock,priceIds:{small:'price_A'}},new AbortController().signal),allow:v=>allowed=v,clock:v=>clock=v,open:()=>openSetup=true,mismatch:()=>amountMismatch=true,pending:()=>paymentPending=true,result:()=>result};
}
...
const publicQuote={id:quote,headquartersId:hq,policyVersion:'fixture',termsRevision:'fixture',amountYen:3300,instructorCount:1,issuedAt:end,expiresAt:end+10000,planKey:'small',planName:'登録講師20名まで',discountDescription:'割引なし',consentRevision:'academy-first-publication-trial-consent-2026-09-08-v1'};
